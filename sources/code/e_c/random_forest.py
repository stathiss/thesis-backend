...
def predict_rf_deepmoji_and_features(add_lexicons=False, add_features=False):
    emotion = None
    ...
    with open('dumps/rf_e_c_x', 'rb') as fp:
        X = pickle.load(fp)

    # y = parse_dataset('E-c', emotion, train_file)[3]
    with open('dumps/rf_e_c_y', 'rb') as fp:
        y = pickle.load(fp)
    ...
    clf = MultiOutputClassifier(RandomForestClassifier(bootstrap=bootstrap,
                                                       max_depth=max_depth,
    ...
    with open('dumps/rf_model' + ("_lexicons" if add_lexicons else "") + ("_features" if add_features else ""), 'wb') as fp:
        pickle.dump(clf, fp)

    predictions = clf.predict(np.array(test_input))
    with open('dumps/rf_predictions' + ("_lexicons" if add_lexicons else "") + ("_features" if add_features else ""), 'wb') as fp:
        pickle.dump(predictions, fp)

    file_name = "./dumps/E-c/" + test_file + "/DeepMoji/E-c_en" + ("_lexicons" if add_lexicons else "") + ("_features" if add_features else "") + "_rf.txt"
...
def write_rf():
    with open('dumps/rf_predictions', 'rb') as fp:
        predictions = pickle.load(fp)

    print(predictions[:10])
    file_name = "./dumps/test_rf.txt"
