...
def predict_svr_deepmoji_and_lexicons(emotion):
    ...
    test_input_extension = read_vectors_from_csv('output.csv')
    print(X_extension[0])
    print(len(X_extension[0]))
    X = np.hstack([X, X_extension])
    print(len(test_input))
    print(len(test_input_extension))
    test_input = np.hstack([test_input, test_input_extension])
