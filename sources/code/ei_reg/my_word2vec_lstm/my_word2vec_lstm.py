...
def get_init_embedding(dictionary):
    glove_file = "sources/features/pretrained_vectors/glove.twitter.27B.50d.txt"
    ...
    word_vec_list = np.zeros(len(dictionary) + 1, 50)
    for v, word in sorted(reversed_dict.items()):
        try:
            word_vec = word_vectors.word_vec(word)
        except KeyError:
            word_vec = np.zeros(50)
        word_vec_list[v] = word_vec
...
def dl_model(emotion):
    ...
    lstm_model.add(Dense(128, kernel_initializer='normal', activation='linear'))

    # The Hidden Layers :
    lstm_model.add(Dense(256, kernel_initializer='normal', activation='linear'))
    lstm_model.add(Dense(256, kernel_initializer='normal', activation='linear'))
    lstm_model.add(Dense(256, kernel_initializer='normal', activation='linear'))
